...
## Статус M1 и frontend F1

Сейчас реализованы первое рабочее backend-ядро и отдельный визуальный этап
frontend:
...
- React/Vite-оболочка и главная сотрудника по утверждённым desktop/mobile
  референсам;
- единый явно маркированный demo fixture без обращения к API, БД или AI;
- адаптивная боковая/нижняя навигация и локально поставляемый шрифт Inter.
...
Frontend F1 является визуальным демо: переходы и выполнение активности
отключены до следующего согласованного этапа.
...
- Node.js 20.19+ либо 22.12+ для frontend на Vite 8;
...
Vite выведет фактический адрес, обычно `http://127.0.0.1:5173/`.
...
Главная показывает только синтетический демонстрационный профиль. Кнопки
деталей и выполнения, а также разделы «Путь» и «История» намеренно отключены:
они входят в F2. Подробнее — в `docs/FRONTEND.md`.
...
- F2-экраны входа, объяснения и результата выполнения;
- подключение frontend к подтверждённому серверному API;

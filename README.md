...
Локальный PostgreSQL при проверке был недоступен. Поэтому миграции, транзакции,
сохранение после перезапуска и параллельное выполнение на PostgreSQL не
запускались. `GET /api/ready` без настроенной БД возвращает 503.
